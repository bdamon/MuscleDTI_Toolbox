...
%      The FOV and slice thickness must be specified in mm.
%
...
for row_cntr = 1:length(roi_mesh_mm(:,1,1))
    inplane_x = squeeze(roi_mesh_mm(row_cntr,:, 1));                                        %find the x positions
    inplane_y = squeeze(roi_mesh_mm(row_cntr,:, 2));                                        %find the y positions
...
for col_cntr = 1:length(roi_mesh_mm(1,:,1))
    thruplane_y = squeeze(roi_mesh_mm(:, col_cntr, 2));                                     %find the y positions
...
for row_cntr = start_row:end_row
    ...
    for col_cntr = start_col:end_col
        ...
        v0=squeeze(roi_mesh_mm(row_cntr, col_cntr,1:3));                                    %v1-4 are the four vertices of the quadrilateral on the roi mesh
        v1=squeeze(roi_mesh_mm(row_cntr+1, col_cntr,1:3));                                  %v1-4 are the four vertices of the quadrilateral on the roi mesh
        v2=squeeze(roi_mesh_mm(row_cntr+1, col_cntr+1,1:3));                                %v1-4 are the four vertices of the quadrilateral on the roi mesh
        v3=squeeze(roi_mesh_mm(row_cntr, col_cntr+1,1:3));                                  %v1-4 are the four vertices of the quadrilateral on the roi mesh
        apo_area(row_cntr, col_cntr) = ...                                                  %model the quadrilateral as two irregular triangles with vertices v0-v1-v2 and v2-v3-v0
            norm(cross((v1-v0), (v2-v1)))/2 + norm(cross((v3-v2), (v3-v0)))/2;              %  their area is 1/2 of the magnitude of the indicated cross products
        ...
        if fiber_length(row_cntr,col_cntr) > 6                                           	%quantify all tracts of at least 6 points
            ...
            for fiber_cntr = 2:fiber_length(row_cntr,col_cntr)
                ...
                %fiber length
                r_diff = squeeze(fiber_all_mm(row_cntr, col_cntr, fiber_cntr, :)) - ...
                ...
                distance_list(row_cntr,col_cntr,fiber_cntr)  =  ...                         %Euclidean distance from preceding point + the preceding point
                    sqrt(sum(r_diff.^2)) + distance_list(row_cntr,col_cntr,fiber_cntr-1);
                ...
                %keep track of nunmber of points quantified.
