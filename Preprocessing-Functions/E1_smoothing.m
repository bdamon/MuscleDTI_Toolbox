...
%  field of the muscle, using the smoothn function developed by Damien
%  Garcia (https://www.mathworks.com/matlabcentral/fileexchange/25634-smoothn)
%  and described in Garcia, Comput. Stat Data Anal., 2010 and Garcia, Exp 
%  Fluids, 2011. The smoothing of E1 for muscle DTI applications was evaluated 
%  in Pineda Guzman et al., NMR Biomed, 2025
%
%INPUT ARGUMENTS
%  E1: A 4D matrix, with the first-third dimensions matching the
...
%
%OUTPUT ARGUMENTS
%  smooth_E1: A 4D matrix, with the first-third dimensions matching the
%   [row column slice] size of the DTI images and the fourth dimension
%   holding the smooth X, Y, and Z components of the first eigenvector
%
%VERSION INFORMATION
...
%  Dependencies: smoothn function by Damien Garcia 
...
for r_img=1:size(E1,1)
    for c_img=1:size(E1,2)
        for s_img=1:size(E1,3)

            E1_vector_smooth = squeeze(smooth_E1(r_img,c_img,s_img,:));

            E1_vector_smooth = E1_vector_smooth/norm(E1_vector_smooth); %Normalize E1

            smooth_E1(r_img,c_img,s_img,:)=E1_vector_smooth;
        end
    end
end
